...
pipeline {
    ...
    environment {
        ...
        ECR_URL = "116981783440.dkr.ecr.ap-northeast-2.amazonaws.com"
                ...
                script {
                    withCredentials([usernamePassword(credentialsId: "${K8S_REPO_CRED}", usernameVariable: "GIT_USERNAME", passwordVariable: 'GIT_PASSWORD')]) {
                        ...
                        sh '''
                            cd ..
                            ls -a
                            git clone https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/kimjiwon0450/orderservice-k8s.git
                        '''
                        ...
                        sh """
                            cd /var/jenkins_home/workspace/orderservice-k8s
                            git config user.name "Jiwon Kim"
                            git config user.email "fresh04502@naver.com"
                            git remote -v
                            git add .
